							...
							<div class="collection-collapse-block-content">
								<div class="collection-brand-filter">
									...
									<div class="custom-control custom-checkbox form-check collection-filter-checkbox">
										<input type="checkbox" name="id_merk[]" value="{{ $merk->id }}" class="custom-control-input form-check-input" id="merk-{{ $merk->id }}" {{ in_array($merk->id, old('id_merk', [])) ? 'checked' : '' }} />
										<label class="custom-control-label form-check-label" for="merk-{{ $merk->id }}">{{ $merk->nama }}</label>
									</div>
									@endforeach
								</div>
							</div>
						...
						<div class="collection-collapse-block open">
							...
							<div class="collection-collapse-block-content">
								<div class="collection-brand-filter">
									...
									<div class="custom-control custom-checkbox form-check collection-filter-checkbox">
										<input type="checkbox" name="id_type[]" value="{{ $type->id }}" class="custom-control-input form-check-input" id="type-{{ $type->id }}" {{ in_array($type->id, old('id_type', [])) ? 'checked' : '' }} />
										<label class="custom-control-label form-check-label" for="type-{{ $type->id }}">{{ $type->nama }}</label>
									</div>
									@endforeach
								</div>
							</div>
							...
							<div class="collection-collapse-block-content">
								<div class="collection-brand-filter mt-3">
									<div class="input-group mb-3">
										...
										<input type="text" class="form-control" id="display_min_price" value="{{ old('min_price') }}" placeholder="Contoh: Rp20.000.000" oninput="formatAndSetRupiah(this, 'hidden_min_price')">
									</div>
									<div class="input-group mb-3">
										<span class="input-group-text">Max</span>
										<input type="text" class="form-control" id="display_max_price" value="{{ old('max_price') }}" placeholder="Contoh: Rp25.000.000" oninput="formatAndSetRupiah(this, 'hidden_max_price')">
									</div>
								</div>
							</div>
						</div>
								...
								<div class="filter-harga-wrapper mt-3">
									<div class="input-group mb-3">
										<select name="sort" class="form-select" aria-label="Sort by">
											...
											<option value="lowest_price" {{ old('sort') == 'lowest_price' ? 'selected' : '' }}>Harga: Rendah ke tinggi</option>
											<option value="highest_price" {{ old('sort') == 'highest_price' ? 'selected' : '' }}>Harga: Tinggi ke rendah</option>
						...
						<div class="row">
							<div class="col-sm-12">
								<div class="top-banner-wrapper">
									<a href="product-page(left-sidebar).html"><img src="./assets/images/motor-terbaru-termurah/All Moto.webp" class="img-fluid" alt="" /></a>
												...
												<div class="product-box">
													<div class="product-imgbox">
														<div class="product-front">
															<img src="{{ asset('assets') }}/images/detail-motor/{{ $motor->detailMotor[0]->gambar }}" class="img-fluid" alt="{{ $motor->detailMotor[0]->gambar }}" />
														</div>
													</div>
