																...
																<tr>
																		...
																		<td>
																				<div class="d-flex justify-content-between">
																						<div class="btn-group">
																								<form action="{{ route('admin.mtr-best-motor.destroy', $ktgMtr->id) }}" method="post">
																										...
																										<button type="button" class="btn btn-primary" data-toggle="modal"
																												data-target="#modalEdit{{ $ktgMtr->id }}">
																												Edit
																										</button>
																										@csrf
																										@method('DELETE')
																						...
																						<!-- Modal update -->
																						<div class="modal fade" id="modalEdit{{ $ktgMtr->id }}" role="dialog"
																								aria-labelledby="myModalLabel">
																								<div class="modal-dialog" role="document">
																										<div class="modal-content">
																												<div class="modal-header">
																														<h4 class="modal-title" id="myModalLabel">Edit data: {{ $ktgMtr->motor->nama ?? 1 }}</h4>
																												</div>
																																...
																																<div class="row">
																																		<div class="form-group col-md-6">
																																				...
																																				<select name="motor" class="form-control select2">
																																						@foreach ($motor as $mtr)
																																								<option value="{{ $mtr->id }}"
																																										@if ($ktgMtr->motor->id ?? 1 === $mtr->id) selected @endif>{{ $mtr->nama }}</option>
																																						@endforeach
																																				</select>
																																		</div>
																																		<div class="form-group col-md-6">
																																				...
																																				<select name="kategori" class="form-control select2">
																																						@foreach ($mtrKategori as $k)
																																								<option value=" {{ $k->id }}"
																																										@if ($ktgMtr->Bestmotor->id === $k->id) selected @endif>{{ $k->nama }}</option>
																																						@endforeach
																																				</select>
																																		</div>
																																</div>
