				...
				<div class="card-body">
					<div class="row">
						<div class="form-group col-md-6">
							...
							@else
							<select id="model" name="model" class="form-control select2 @error('model') is-invalid @enderror" style="width: 100%;">
								<option value="" selected>-- Pilih motor --</option>
								@foreach ($dataMotor as $m)
								<option value="{{ $m->id }}" {{ old('model') == $m->id ? 'selected' : '' }}>{{ $m->nama }}</option>
								@endforeach
							</select>
							@endif
							@error('model')
						...
						<div class="form-group col-md-6">
							...
							<input name="warna-motor" type="text" class="form-control @error('warna-motor') is-invalid @enderror" id="warna-motor" placeholder="Masukan warna motor" value="{{ old('warna-motor') }}">
							@error('warna-motor')
							<div class="alert alert-danger">{{ $message }}</div>
							@enderror
						</div>
