			...
			<form role="form" action="{{ route('admin.hook.store') }}" method="POST" enctype="multipart/form-data">
				...
				<div class="card-body">
					...
					<div class="form-group col-md-6">
						...
						<p>Silhakn pilih status hook, default aktif</p>
						<select name="status-hook" class="form-control">
							<option value="" selected>-- Pilih status hook --</option>
							<option value="1">Aktif</option>
							<option value="0">Tidak Aktif</option>
						</select>
					</div>
					<div class="form-group col-md-6">
						<label for="warna_teks">Urutan Hook</label>
						<input type="number" class="form-control" id="urutan-hook" name="urutan-hook" placeholder="Masukan urutan hook">
					</div>
					<div class="form-group">
				...
				<table class="table-hover table">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{{ $hook->status === 1 ? 'Aktif' : 'Tidak Aktif' }}
							</td>
							<td>
								{{ $hook->order }}
							</td>
												...
												<div class="modal-body">
													<div class="card-body">
														...
														<div class="form-group">
															...
															<select name="status-hook" class="form-control">
																<option value="" selected>-- Pilih status hook --</option>
																<option value="1" {{$hook->status === 1 ? 'selected' : ''}}>Aktif</option>
																<option value="0" {{$hook->status === 0 ? 'selected' : ''}}>Tidak Aktif</option>
															</select>
														</div>
														<div class="form-group">
															<label for="warna_teks">Urutan Hook</label>
															<input type="number" class="form-control" id="urutan-hook" name="urutan-hook" placeholder="Masukan urutan hook" value="{{$hook->order}}">
														</div>
														<div class="form-group">
															<label for="gambar">File input</label>
															<input type="file" id="gambar" name="gambar">
